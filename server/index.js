...
app.post('/api/export-pdf', authMiddleware, async (req, res) => {
  try {
    ...
    // Clean up Markdown so it looks good in plain text PDF
    const cleanSummary = summary
      ? summary
          .replace(/\*\*/g, '')           // Remove bold asterisks
          .replace(/^#+\s+/gm, '')        // Remove Markdown headers (#, ##, etc.)
          .replace(/^[\*\-]\s+/gm, '• ')  // Replace bullet points (* or -) with a clean bullet
      : '';

    doc.moveDown(4);
    doc.fillColor('#111111').fontSize(13).text(cleanSummary, { lineGap: 6, align: 'left' });
    doc.end();
  } catch (err) {
